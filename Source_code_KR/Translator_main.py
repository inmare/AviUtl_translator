...
def helper_gui(root):
    ...
    #스크립트 넣는 곳 라벨
    help_input_label = tk.Label(root, text="스크립트 입력", font=font_UI)
    ...
    #추출된 글자 표시하는 곳 라벨
    help_output_1_label = tk.Label(root, text="추출된 글자", font=font_UI)
    ...
    #바꿀 글자 넣을 곳 라벨
    help_input_2_label = tk.Label(root, text="바꿀 글자", font=font_UI)
    ...
    #결과 라벨 
    help_output_label = tk.Label(root, text="변환된 스크립트", font=font_UI)
    ...
    #결과 상태 표시 라벨
    help_res_lab = tk.Label(root, text="변환 결과", font=font_UI)
    help_res_lab.place(x=10, y=400)
    #결과 상태 표시 항목 라벨
    change_res_text = tk.StringVar()
    help_res_lab_item = tk.Label(root, justify="left", textvariable=change_res_text, font=font_Entry)
    help_res_lab_item.place(x=10, y=420)

    #검사 결과 라벨
    help_chk_lab = tk.Label(root, text="스크립트 검사 결과", font=font_UI)
    help_chk_lab.place(x=10, y=460)
    #검사 결과 항목 라벨
    chk_lab_text = tk.StringVar()
    help_chk_lab_item = tk.Label(root, justify="left", textvariable=chk_lab_text, font=font_Entry)
    help_chk_lab_item.place(x=10, y=480)
    ...
    #추출 버튼
    help_extract_button = tk.Button(root, text="글자 추출", font=font_UI, command=lambda: py_sec.main_extract(help_dict))
    help_extract_button.place(x=470, y=7, width=73, height=40)
    #체크 버튼
    help_check_button = tk.Button(root, text="검사", font=font_UI, command=lambda: py_sec.main_check(help_dict))
    help_check_button.place(x=552, y=7, width=73, height=40)
    #변환 버튼
    help_change_button = tk.Button(root, text="변환", font=font_UI, command=lambda: py_sec.main_get_change(help_dict))
    help_change_button.place(x=633, y=7, width=73, height=40)
    #초기화 버튼
    help_init_button = tk.Button(root, text="초기화", font=font_UI, command=lambda: py_sec.initialize_text(help_dict))
    help_init_button.place(x=714, y=7, width=73, height=40)
    #추출한 글자 복사 버튼
    help_copy_ext_button = tk.Button(root, text="추출된 글자 복사", font=font_UI, command=py_sec.extract_copy)
    help_copy_ext_button.place(x=470, y=50, width=155, height=30)
    #결과 복사 버튼
    help_copy_res_button = tk.Button(root, text="결과 복사", font=font_UI, command=py_sec.result_copy)
    help_copy_res_button.place(x=633, y=50, width=155, height=30)
    #번역기 창 버튼
    help_init_button = tk.Button(root, text="구글 번역기\n바로가기", font=font_UI, command=py_sec.open_google)
...
def eff_trans_gui(root):
    ...
    #입력하는 곳 라벨
    eff_label_input = tk.Label(root, text="스크립트 입력", font=font_UI)
    eff_label_input.place(x=10, y=7)
    #결과 라벨
    eff_label_result = tk.Label(root, text="번역 결과", font=font_UI)
    ...
    #번역 결과 라벨
    eff_alert_label = tk.Label(root, text="번역 결과", font=font_UI)
    ...
    #추가 설명
    eff_recommend = tk.Label(root, justify="left", text="몇몇 효과는 제대로 번역되지 않을 수 있습니다.\n그럴 땐 효과 사전에서 효과가 있는지 검색해주세요.", font=font_UI)
    ...
    #번역하는 버튼
    eff_button_input = tk.Button(root, text="번역", font=font_UI, command=lambda: py_fir.main_change(eff_trans_dict))
    eff_button_input.place(x=690, y=30, width=100, height=40)
    #초기화하는 버튼
    eff_button_clear = tk.Button(root, text="초기화", font=font_UI, command=lambda: py_fir.initialize(eff_trans_dict))
    eff_button_clear.place(x=690, y=75, width=100, height=40)
    #복사하는 버튼
    eff_button_copy = tk.Button(root, text="결과 복사", font=font_UI, command=lambda: py_fir.copy_effect(eff_trans_dict))
    eff_button_copy.place(x=690, y=120, width=100, height=40)
    #효과 사전 버튼
    eff_dict_copy = tk.Button(root, text="효과 사전\n바로가기", font=font_UI, command=py_fir.open_web)
...
if __name__ == '__main__':
    ...
    font_UI = tk.font.Font(family="맑은 고딕", size=10, weight="bold")
    font_Entry = tk.font.Font(family="맑은 고딕", size=10)
    menubar = tk.Menu(root)
    helper_gui(root)
    menubar.add_command(label="제목 번역하기", command=lambda: helper_gui(root))
    menubar.add_command(label="효과 번역하기", command=lambda: eff_trans_gui(root))
