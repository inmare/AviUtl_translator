...
def main_extract(helper):
    ...
    if info_dict["track_num"] == 0 and info_dict["check_exist"] == False and info_dict["dialog_exist"] == False:
        helper["output1"].configure(state="normal")
        helper["output1"].insert("1.0", "잘못된 스크립트를\n입력했습니다.\n올바른 스크립트를\n입력해주세요.")
...
def main_check(helper):
    ...
    if chk_num[0]:
        return helper["check"].set(f"- 설정 항목의 개수가 일치 하지 않습니다. {chk_num[1]}개의 항목을 입력해주세요.")
    ...
    for i in range(len(en_string_list)):
        if len(en_string_list[i]) > 16:
            text_to_send = text_to_send + "- 단어의 길이가 너무 길어서 AviUtl에서 제대로 보이지 않을 수 있습니다.\n16글자 이하로 길이를 줄이는 것을 추천합니다.(변환에는 문제가 없습니다.)\n"
            break
    ...
    for i in range(len(en_string_list)):
        if check_improper_word(i, en_string_list):
            return helper["check"].set("- \",\", \"/\"같은 글자들은 스크립트의 오작동을 일으킬 수 있습니다.\n다른 글자로 바꾸어주세요.([/] 와 같이 대괄호 안에 /이 있는 건 괜찮습니다.)\n")
        for j in (range(len(non_proper_word))):
            if non_proper_word[j] in en_string_list[i]:
                text_to_send = text_to_send + "- \"└\", \"↑\"같은 글자나 전각문자\"（\", \"［\", \"～\"은 스크립트에서 뷁어로 보일 수 있습니다.\n다른 글자로 대체해주세요.(\"└\"나 \"↑\"를 \">\"로 바꾸는 걸 권장합니다. 또한 변환에는 문제가 없습니다.)\n"
                break
    
    if text_to_send == "":
        text_to_send = text_to_send + "- 변환해도 괜찮습니다."
...
def main_get_change(helper):
    ...
    if chk_num[0]:
        helper["check"].set("")
        return helper["result"].set(f"- 설정 항목의 개수가 일치 하지 않습니다. {chk_num[1]}개의 항목을 입력해주세요.")
    #가동을 아예시키지 못하는 함수가 있어도 중지시킴
    for i in range(len(en_string_list)):
        if check_improper_word(i, en_string_list):
            helper["check"].set("")
            return helper["result"].set("- \",\", \"/\"같은 글자들은 스크립트의 오작동을 일으킬 수 있습니다. 다른 글자로 바꾸어주세요.\n([/] 와 같이 대괄호 안에 /이 있는 건 괜찮습니다.)\n")
