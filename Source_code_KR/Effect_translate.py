...
def change_title():
    global main_string
    fir_loc = main_string.find("\"")+1
    sec_loc = main_string.find("\"", fir_loc)
    eff_name = main_string[fir_loc:sec_loc]
    for i in range(len(eff_list_JP)):
        if eff_list_JP[i] == eff_name:
            main_string_list = list(main_string)
            main_string_list[fir_loc:sec_loc] = eff_list_EN[i]
            main_string = "".join(main_string_list)
            return i
        elif not (eff_list_JP[i] == eff_name) and i == len(eff_list_JP)-1:
            return -1
...
def main_change(effTrans):
    ...
    if shape_in_it == -1: #효과를 잘못 입력했을 때
        ...
        effTrans["output"].delete("1.0", "end")
        effTrans["output"].configure(state="disabled")        
        return effTrans["alert"].set("프로그램에 있는 효과와 일치하는 효과이름이 없거나 스크립트 전체가 영어로 되어있습니다. 스크립트를 확인해주세요.")
    elif shape_in_it != -1 and shape_in_it >= len(eff_list_JP)-7: #입력한 효과가 도형일 때
        effTrans["output"].configure(state="normal")
        effTrans["output"].delete("1.0", "end")
        effTrans["output"].insert("1.0", main_string)
        effTrans["output"].configure(state="disabled")
        return effTrans["alert"].set("번역이 끝났습니다.")
